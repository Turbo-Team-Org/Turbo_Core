...
class PlaceCategoryServiceSupabase implements PlaceCategoryRepositoryInterface {
  ...
  Future<bool> updatePlaceCategories(
    ...
    try {
      ...
      final currentCategoryIds = currentResponse
      ...
      final newRelationships = categoryIds.map((categoryId) => {
        'place_id': placeId,
        'category_id': categoryId,
        'created_at': DateTime.now().toIso8601String(),
      }).toList();
  ...
  Future<List<Place>> getPlacesInCategory(String categoryId) async {
    try {
      ...
      return placesResponse.map<Place>((data) => _placeFromSupabase(data)).toList();
    } catch (e) {
